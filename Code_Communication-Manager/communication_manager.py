...
def forward_main_ambulance_id():
    try:
        ...
        rsm_res = req.post(
            "http://rsm:5003/get_main_ambulance_id", json={"isan": data["isan"]}
        )
        if rsm_res.status_code == 204 or rsm_res.status_code == 200:
            ...
            if ambulance_id:
                ...
                if cs_res.status_code == 204 or cs_res.status_code == 200:
                    ...
                    logging.error(
                        f"Failed to forward data to CS. Status Code: {cs_res.status_code}, Response: {cs_res.text}"
                    )
                    return {
                        "status": "error",
            ...
            logging.error(
                f"Failed to get ambulance ID from RSM. Status Code: {rsm_res.status_code}, Response: {rsm_res.text}"
            )
            return {
                "status": "error",
    ...
    try:
        ...
        if len(ACTIVE_CS_IPS) == 1:
            ...
            thread_one = threading.Thread(
                target=periodic_request_occupied_ambulances_ids_or_ips, daemon=True
            )
            thread_one.start()
            time.sleep(1)
            if TRACKING_SIMULATION == 1:
                thread_two = threading.Thread(
                    target=simulation_request_occupied_ambulances_coordinates,
                    daemon=True,
                )
                thread_two.start()
            elif TRACKING_SIMULATION == 0:
                thread_two = threading.Thread(
                    target=practical_request_occupied_ambulances_coordinates,
                    daemon=True,
                )
                thread_two.start()
        return {
                ...
                if response.status_code == 200:
                    ...
                    if position:
                        if "isAtHospital" in position:
                            ...
                            delete_command = (
                                "DELETE FROM activeCom WHERE respon_adr = %s"
                            )
...
def simulation_request_occupied_ambulances_coordinates():
    ...
    try:
        for rs_ip in PRACTICAL_OCCUPIED_RS_IPS:
            ...
            post_start_response = req.post(
                f"http://{rs_ip}:{port}/simulation_start_tracking_single_ambulance"
            )
        ...
        while ACTIVE_CS_IPS:
                ...
                if new_ambulances_ips:
                    for ambulance_ip in new_ambulances_ips:
                        if ambulance_ip == "172.18.0.12":
                            ...
                            continue  # IP is not recognized
                        post_start_response = req.post(
                            f"http://{ambulance_ip}:{port}/simulation_start_tracking_single_ambulance"
                        )
                        if post_start_response.status_code != 204:
                            print(
            ...
            for ambulance_ip in list(local_active_ambulances_ips):
                if ambulance_ip == "172.18.0.12":
                    ...
                    continue  # IP is not recognized
                response = req.get(
                    f"http://{ambulance_ip}:{port}/simulation_current_location_single_ambulance"
                )
                if response.status_code == 200:
                    position = response.json().get("position")
                    if position:
                        if "isAtHospital" in position:
                            ...
                            rs_res = req.post(
                                f"http://{ambulance_ip}:{port}/simulation_stop_particular_thread"
                            )
                        positionsSet.append(position)
                else:
                    print(
...
def handle_exit_cs():
    ...
    try:
        ...
        if req_sys_ip in ACTIVE_CS_IPS:
            ...
            return {"status": "error", "message": "CS not in ACTIVE_CS_IPS List."}, 404
            ...
            if TRACKING_SIMULATION == 1:
                ...
                for rs_ip in local_active_ambulances_ips:
                    if rs_ip == "172.18.0.12":
                        ...
                        continue  # IP is not recognized
                    response = req.post(
                        f"http://{rs_ip}:{port}/simulation_stop_particular_thread"
                    )
            elif TRACKING_SIMULATION == 0:
        ...
        return {"status": "success", "message": "CS removed successfully."}, 200
    except Exception as e:
        logging.error(
            f"An error occurred during the CS exit handling process: {str(e)}"
        )
        return {"status": "error", "message": f"An error occurred: {str(e)}"}, 500
    ...
    try:
        ...
        for ip in ACTIVE_CS_IPS:
            thread = threading.Thread(
                target=thread_send_broken_ambulance_id_to_cs, args=(ip, ambulance_id)
            )
            thread.start()
        return {
            "status": "success",
        ...
        logging.error(
            f"An error occurred during forwarding broken ambulance id : {str(e)}"
        )
        return {"status": "error", "message": f"An error occurred: {str(e)}"}, 500
